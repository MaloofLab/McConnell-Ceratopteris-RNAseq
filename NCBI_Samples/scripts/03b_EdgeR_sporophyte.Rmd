...
### Filter for sporophyte only

```{r}
sample.description.Mc <- sample.description.Mc %>% filter(str_detect(tissue, "gam|herm", negate = TRUE))
...
```{r}
sample.description.ncbi <- sample.description.ncbi %>% filter(str_detect(stage, "sporophyte"))
...
write_csv(sample.description, "../output/sample.description.sporophyte.all.csv")
...
```{r}
dge.data %>% cpm(log = TRUE) %>% write.csv("../output/sporophyte_combined_log2cpm.csv")
R.utils::gzip("../output/sporophyte_combined_log2cpm.csv", overwrite=TRUE)
```
