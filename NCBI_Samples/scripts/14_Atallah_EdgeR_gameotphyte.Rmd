...
sample.description <- sample.description.ncbi %>% 
  ...
  mutate(trt = ifelse(str_detect(description, "antheridogen"), "antheridogen", "ctrl")) %>%
...
colnames(counts.data)[-1] <- str_c(sample.description$sample, "_", sample.description$trt)
```
...
Note: up = ctrl, down = with antheridogen.  This might seem backwards but it matches the direction of the way I did Hannah's
...
```{r}
genes.up.antheridogen <- genes.all %>% mutate(up = (FDR <= 0.01 & logFC < 0) * 1 ) %>% pull(up, name = geneID)
genes.down.antheridogen <- genes.all %>% mutate(up = (FDR <= 0.01 & logFC > 0) * 1 ) %>% pull(up, name = geneID)
```
...
nullp.result.up <- nullp(DEgenes = genes.up.antheridogen, bias.data = gene.lengths)
...
```{r}
#determines if there is bias due to gene length.  The plot shows the relationship.
nullp.result.down <- nullp(DEgenes = genes.down.antheridogen, bias.data = gene.lengths)
