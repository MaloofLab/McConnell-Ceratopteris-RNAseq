...
The 4.5 DAG samples are strong outliers, so will remove them.
...
sample.description <- sample.description.ncbi %>% 
  ...
  filter(str_detect(label, fixed("4.5 DAG"), negate = TRUE))

counts.data <- full_join(counts.data.ncbi, counts.data.Mc)

counts.data <- counts.data[,c("gene_ID", sample.description$sample)]

colnames(counts.data)[-1] == sample.description$sample

write_csv(sample.description, "../output/sample.description.gametophyte.no4.5DAG.csv")
```
...
```{r}
dge.data %>% cpm(log = TRUE) %>% write.csv("../output/gam_combined_log2cpm_no4.5DAG.csv")
R.utils::gzip("../output/gam_combined_log2cpm_no4.5DAG.csv", overwrite=TRUE)
```
