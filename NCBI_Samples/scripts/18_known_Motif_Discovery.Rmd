...
results.table <- results.table %>%
  ...
  mutate(
    p.value = fisher.test(
      matrix(c(
        target.count,
        length(cluster_seqs) - target.count,
        non.target.count,
        length(background_seqs) - non.target.count
      ), ncol = 2),
      alternative = "greater"
    )$p.value,
    non.target.percent = round(non.target.count / length(background_seqs), 3) * 100,
    target.percent = round(target.count / length(cluster_seqs), 3) * 100
  ) %>%
  ungroup() %>%
