...
```{r}
counts.data[-1] %>% colSums()
counts.data[-1] %>% colSums() %>% mean()
```

```{r}
counts.data %>% filter(str_detect(gene_ID, "__")) %>% magrittr::extract(-1) %>% colSums()
counts.data %>% filter(str_detect(gene_ID, "__")) %>% magrittr::extract(-1) %>% colSums() %>% mean()
```
...
```{r}
write_csv(counts.data, "../output/counts_unfiltered_male_herm.csv.gz")
```
...
```{r}
mdsvals2 <- tibble(MDS1=mdsvals$x, MDS2=mdsvals$y, sample=rownames(dge.data$samples)) %>%
  inner_join(sample.description) %>%
  mutate(sample = ifelse(tissue == "male_gam", "male", "hermaphrodite"))

var.explained <- mdsvals$var.explained[1:2] %>% magrittr::multiply_by(100) %>% round(1) %>% str_c(" (", ., "%)")


mdsvals2 %>%  ggplot(aes(x=MDS1, y=MDS2, color=sample, shape = sample)) +
  scale_color_brewer(type="qual", palette = 3) +
  theme_bw() +
  geom_point(size=3) +
  labs(x = str_c("Dimension 1", var.explained[1]),  y = str_c("Dimension 2", var.explained[2]))

ggsave("../output/McConnellMDS.png")

ggsave("../output/McConnellMDS.pdf")
...
dev.copy(png, filename = "../output/male_v_herm_heatmap_tall.png", width=1200, height = 1800)

genes.heatmap.reorder <- genes.heatmap[rev(match(colnames(hm$carpet),genes.heatmap$geneID)),]

write_csv(genes.heatmap.reorder, "../output/male_v_herm_heatmap_genes_ordered.csv")
...
dev.copy(png, filename = "../output/male_v_herm_heatmap_tall_centered.png", width=1200, height = 1800)

genes.heatmap.reorder.centered <- genes.heatmap[rev(match(colnames(hm.centered$carpet),genes.heatmap$geneID)),]

write_csv(genes.heatmap.reorder.centered, "../output/male_v_herm_heatmap_genes_ordered_centered.csv")
...
saveRDS(gene.lengths, "../../shared/input/genelengths.Rds")
```
