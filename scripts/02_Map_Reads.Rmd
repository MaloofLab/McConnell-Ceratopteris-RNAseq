...
for f1 in $R1_files
  do
    ...
    samtools sort -m 5G -O BAM --write-index --threads 4 ${out_file}Aligned.out.bam > ${out_file}Aligned.out.sorted.bam
    samtools sort -m 5G -O BAM --write-index --threads 4 ${out_file2}Aligned.out.bam > ${out_file2}Aligned.out.sorted.bam
...
htseq-count --stranded=no --idattr=Parent --order=pos *.bam $gff -n 30 > $out_file

# Note that the paired and unpaired reads are counted separartely for each file and will need to be combined.
```
